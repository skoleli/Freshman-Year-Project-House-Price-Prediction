...
//Parametre olarak verilen tek bir ev degerinin tum ozelliklerini ekrana bastitir.
...
void mean_sale_prices(House ** houses, int size, char* criter_name){
   ...
   quicksort(houses, 0, size-1, criter_name);
    ...
    float* mean = (float*) malloc(sizeof(float)*capacity);
    float *sum = (float*) calloc(capacity, sizeof(float));
    float *miktar = (float*) calloc(capacity, sizeof(float));
    j = 0;
